    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card mb-6">
            ...
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('dashboard.episodes.update', $episode->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row p-5">
                        <h3>{{ __('Edit Episode') }}</h3>
                        <div class="mb-4 col-md-6">
                            <label for="name" class="form-label">{{ __('Name') }}</label><span
                                class="text-danger">*</span>
                            <input class="form-control @error('name') is-invalid @enderror" type="text" id="name"
                                name="name" required placeholder="{{ __('Enter name') }}" autofocus value="{{old('name', $episode->namaSiswa)}}"/>
                            @error('name')
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <input class="form-control @error('episode') is-invalid @enderror" type="text" id="episode"
                                name="episode" required placeholder="{{ __('Enter episode name') }}" autofocus value="{{old('episode', $episode->episode)}}"/>
                            @error('episode')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                                ...
                                @if (isset($courses) && count($courses) > 0)
                                    @foreach ($courses as $course)
                                        <option value="{{ $course->id }}"
                                            {{ $course->id == old('course_id', $episode->course_id) ? 'selected' : '' }}>{{ $course->name }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                                ...
                                @if (isset($subcourses) && count($subcourses) > 0)
                                    @foreach ($subcourses as $subcourse)
                                        <option value="{{ $subcourse->id }}"
                                            {{ $subcourse->id == old('sub_course_id', $episode->sub_course_id) ? 'selected' : '' }}>{{ $subcourse->name }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <input class="form-control @error('folder') is-invalid @enderror" type="text" id="folder"
                                name="folder" required placeholder="{{ __('Enter link') }}" autofocus value="{{old('folder', $episode->folder)}}"/>
                            @error('folder')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="col-md-6 mb-4">
                            <label class="switch switch-square">
                                ...
                                <input type="checkbox" class="switch-input @error('is_new') is-invalid @enderror" id="is_new"
                                    name="is_new" {{ old('is_new', $episode->is_new == 1) ? 'checked' : '' }}>
                    ...
                    <div class="mt-2">
                        <button type="submit" class="btn btn-primary me-3">{{ __('Update Episode') }}</button>
                    </div>
                </form>
            </div>
            <!-- /Account -->
