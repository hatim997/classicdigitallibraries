        ...
        <div class="card mb-6">
            ...
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('dashboard.courses.update', $course->id) }}" enctype="multipart/form-data">
                    ...
                    @method('PUT')
                    <div class="row p-5">
                        <h3>{{ __('Edit Course') }}</h3>
                        <div class="mb-4 col-md-6">
                            <label for="name" class="form-label">{{ __('Name') }}</label><span
                                class="text-danger">*</span>
                            <input class="form-control @error('name') is-invalid @enderror" type="text" id="name"
                                name="name" required placeholder="{{ __('Enter name') }}" autofocus value="{{old('name', $course->name)}}"/>
                            @error('name')
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            @enderror
                            @if($course->image)
                                <img src="{{ asset('courses/'.$course->image) }}" alt="Image" class="mt-2" width="120">
                            @endif
                        </div>
                    </div>
                    <div class="mt-2">
                        <button type="submit" class="btn btn-primary me-3">{{ __('Update Course') }}</button>
                    </div>
                </form>
            </div>
            <!-- /Account -->
